...
# Continuously collect Prometheus, HPA, and deployment metrics
while true; do
  ts="$(date -Iseconds)"

  # Get Prometheus PPS value
  pps="$(prom_query || true)"
  pps="${pps:-NA}"

  # Get HPA replica and target metrics
  ...
  # Get deployment readiness and replica count
  ...
  # Append collected metrics to CSV
